...
import com.huan.jerp.data.Work;
...
public class CalendarView extends LinearLayout implements OnClickListener {
	...
	private void initView() {
		...
		final Handler myHandler=new Handler(){
			...
			public void handleMessage(Message msg) {
				try {
					// ----
					if (msg.what == 0) {
						List<Work> list=(List<Work>)msg.obj;
						System.out.println("list:"+list.size());

						calV = new CalendarAdapter(mContext, getResources(), jumpMonth,
								jumpYear, year_c, month_c, day_c,list.get(0).toBoolean());
						addGridView();
		...
		BmobQuery<Work> w=new BmobQuery<Work>();
		w.addWhereEqualTo("user_id", BmobUser.getCurrentUser(this.getContext(), User.class).getObjectId());
		w.findObjects(this.getContext(), new FindListener<Work>() {
			@Override
			public void onSuccess(List<Work> list) {
				...
				message.obj = list;
				//data.putParcelableArrayList("yr",list);
